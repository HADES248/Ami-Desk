...
export default function Navbar() {
  ...
  return (
    <nav className="relative align-center lg:pb-4">
      <Image src={Logo} alt="Helpdesk" width={100} quality={100} placeholder="blur" />
      <div className="absolute right-0 top-20 text-right hidden lg:inline-block lg:left-16 lg:top-8 lg:right-auto" id="navlinks">
        <h1 className="block lg:inline-block pl-6">Ami-Desk</h1>
      ...
      </div>
      {
        user ? (
          ...
          <>
            <Link className="absolute ml-20 top-9 text-sm lg:right-2" href={'/components/signUp'}>
              <h2>Sign Up</h2>
            </Link>
            <Link className="absolute ml-40 top-9 lg:right-24" href={'/components/login'}>
              <h2>Log In</h2>
            </Link>
          </>
        )
